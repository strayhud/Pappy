...
*/
	...
	*/
	...
	/*
	**	buildPanels()
	**		Construct gallery panels from <div class="gv-panel"> elements
	...
	*/
		return this.each(function() {
			...
			_t.wrap("<div></div>");
			j_gallery = _t.parent();
			j_gallery.css('visibility','hidden').attr('id',_t.attr('id')).addClass('gv-gallery');
			...
			if(opts.show_panels) {
				for(i=j_frames.length-1;i>=0;i--) {
					if(j_frames.eq(i).find('.gv-panel-content').length>0) {
						j_frames.eq(i).find('.gv-panel-content').remove().prependTo(j_panel_wrapper).addClass('gv-panel');
					} else {
						p = $('<div>');
						p.addClass('gv-panel');
						im = $('<img />');
